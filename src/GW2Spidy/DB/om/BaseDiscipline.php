...
abstract class BaseDiscipline extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'GW2Spidy\\DB\\DisciplinePeer';
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[] = DisciplinePeer::NAME;
        }
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = DisciplineQuery::create()
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->recipesScheduledForDeletion !== null) {
                if (!$this->recipesScheduledForDeletion->isEmpty()) {
                    foreach ($this->recipesScheduledForDeletion as $recipe) {
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        $sql = sprintf(
            'INSERT INTO `discipline` (%s) VALUES (%s)',
    ...
    {
        if (isset($alreadyDumpedObjects['Discipline'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Discipline'][$this->getPrimaryKey()] = true;
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collRecipes) {
                $result['Recipes'] = $this->collRecipes->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
     ...
     * @return void
     */
    public function resetPartialRecipes($v = true)
    {
     ...
     * @return void
     */
    public function initRecipes($overrideExisting = true)
    {
    ...
    /**
     * Sets a collection of Recipe objects related by a one-to-many relationship
    ...
    {
        $this->recipesScheduledForDeletion = $this->getRecipes(new Criteria(), $con)->diff($recipes);

        foreach ($this->recipesScheduledForDeletion as $recipeRemoved) {
            $recipeRemoved->setDiscipline(null);
        }
     ...
     * @throws PropelException
     */
    public function countRecipes(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collRecipesPartial && !$this->isNew();
        if (null === $this->collRecipes || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collRecipes) {
    ...
    public function addRecipe(Recipe $l)
    {
        ...
        if (!$this->collRecipes->contains($l)) { // only add it if the **same** object is not already associated
            $this->doAddRecipe($l);
        }
    ...
    /**
     * @param	Recipe $recipe The recipe object to remove.
     */
    ...
    {
        return (string) $this->exportTo(DisciplinePeer::DEFAULT_STRING_FORMAT);
    }
