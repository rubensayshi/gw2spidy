<?php

namespace GW2Spidy\DB\map;

use \RelationMap;
use \TableMap;


/**
 * This class defines the structure of the 'recipe_ingredient' table.
 *
 *
 *
 * This map class is used by Propel to do runtime db structure discovery.
 * For example, the createSelectSql() method checks the type of a given column used in an
 * ORDER BY clause to know whether it needs to apply SQL to make the ORDER BY case-insensitive
 * (i.e. if it's a text column type).
 *
 * @package    propel.generator.gw2spidy.map
 */
class RecipeIngredientTableMap extends TableMap
{

    /**
     * The (dot-path) name of this class
     */
    const CLASS_NAME = 'gw2spidy.map.RecipeIngredientTableMap';

    /**
     * Initialize the table attributes, columns and validators
     * Relations are not initialized by this method since they are lazy loaded
     *
     * @return void
     * @throws PropelException
     */
    public function initialize()
    {
        // attributes
        $this->setName('recipe_ingredient');
        $this->setPhpName('RecipeIngredient');
        $this->setClassname('GW2Spidy\\DB\\RecipeIngredient');
        $this->setPackage('gw2spidy');
        $this->setUseIdGenerator(false);
        $this->setIsCrossRef(true);
        // columns
        $this->addForeignPrimaryKey('RECIPE_ID', 'RecipeId', 'INTEGER' , 'recipe', 'DATA_ID', true, null, null);
        $this->addForeignPrimaryKey('ITEM_ID', 'ItemId', 'INTEGER' , 'item', 'DATA_ID', true, null, null);
        $this->addColumn('COUNT', 'Count', 'INTEGER', true, null, 1);
        // validators
    } // initialize()

    /**
     * Build the RelationMap objects for this table relationships
     */
    public function buildRelations()
    {
        $this->addRelation('Recipe', 'GW2Spidy\\DB\\Recipe', RelationMap::MANY_TO_ONE, array('recipe_id' => 'data_id', ), null, null);
        $this->addRelation('Item', 'GW2Spidy\\DB\\Item', RelationMap::MANY_TO_ONE, array('item_id' => 'data_id', ), null, null);
    } // buildRelations()

} // RecipeIngredientTableMap
