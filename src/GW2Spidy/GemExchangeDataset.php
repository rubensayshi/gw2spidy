...
class GemExchangeDataset {
    ...
    protected function updateDataset() {
        ...
        $q->filterByRateDatetime(date("Y-m-d 00:00:00", strtotime("-1 week")), \Criteria::GREATER_EQUAL);
        if ($start) {
        ...
        foreach ($rates as $rateEntry) {
            ...
            $this->raw[] = array($date->getTimestamp() * 1000, intval($rate));
        ...
        $end = end($rates);
        $end = new DateTime("{$rateEntry['rateDatetime']}");
